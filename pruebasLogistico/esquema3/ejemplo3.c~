/*Este programa utiliza la operacion XOR de manera aleatorio. 
Tal como fue enviado. LOs datos generados se guardan en archibos
para su graficacion.*/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int main() {
	...
	Par[0] = 3.9768; Par[1] = 3.9553; Par[2] = 3.9368;  Par[3] = 3.99041;
	Var[0] =  0.4057062; Var[1] =  0.1057062; Var[2] =  0.8057062; Var[3] =  0.57062;
	Par[4] = 3.9989; Par[5] = 3.965; Par[6] = 3.9999;  Par[7] = 3.918;
	...
	while(i < TotalDatos) {  
		for(tempH = 0, k=0; k < numMaps; k++) {
			X[i] = EPS*LogisticMap(Par[m],X[i-m]) + eps*H;
			printf("\n@@@ X: %f   m: %u  i-m:%u     i:%u",X[i],m,i-m,i);
