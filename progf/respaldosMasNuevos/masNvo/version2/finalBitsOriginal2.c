...
-El metodo de cifrado es el de VERONICA: operaciones ACOPLADAS.
...
int main(){
	...
	for(irtp=0; irtp < numPaquetesRTP; irtp++ ){
		...
		for(iSeg =NUMSEGMENTOS; iSeg >0; iSeg--){
			...
			bitRef=0;	
		        epsilon = (H & 15);
			while((bitRef+BF) <numBitsSegmento){
				...
				/*Si el un numero aleatorio es par...*/
                      	 	...
                      	 	datosAuxiliar[pos_byte + (posicionInversion>>3)]^=flip[posicionInversion & 7];  
                        	count++;  
