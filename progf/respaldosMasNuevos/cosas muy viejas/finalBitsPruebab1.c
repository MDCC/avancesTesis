...
como referencia el ultimo bit invertido (lo que dice en las observaciones), pero ahora
se utiliza una versión más simplificada para la inversión de bits.*/
...
int main(){
	...
	for(irtp=0; irtp < numPaquetesRTP; irtp++ ){
		...
		contadorMapa=0;
			...
			datosArchivo[refRTP  + (bitRef/8)]^=(1<<(avCodigo-(bitRef&7)-1));
		...
		for(iSeg =Lsegmentos; iSeg >1; iSeg--){
			...
			segmentos[T] = auxiliar;
		        ...
		        /*Nos posicionamos en el segmento que queda en el final.*/
                        /*Esta instruccion que onda, todavia no se que hacer con ella*/
