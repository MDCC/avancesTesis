	...
	imprimirArreglo(textos, tamanioArreglo);
		...
		printf("\n El valor aleatorio es  %u  \n", aleatorio);
 			...
 			//if(de!=0){	
				...
				posicionInversion=(aleatorio%(avCodigo -(bitRef+1))  )+1;
				bitRef = (bitRef+posicionInversion);
			
		                textos[0+irtp*tamRTP]= textos[0+irtp*tamRTP]^(1<<(avCodigo -bitRef-1));
			...
			//if(db!=0){
				...
				//Estos son los step y el pos_F.
				posicionInversion=(aleatorio % bitRef)+1;
			        bitRef = (bitRef-posicionInversion);				
